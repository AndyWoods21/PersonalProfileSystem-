...
/**
 * Servlet handling user authentication, email verification validation,
 * and loading complete profile data into the session for CV rendering and dashboard display.
 */
...
public class LoginServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (user != null && user.getPasswordHash() != null && user.getPasswordHash().equals(password)) {
                ...
                // Gatekeeper: Check if user has verified their email address
                if (!user.isVerified()) {
                    request.setAttribute("errorMessage", "Please verify your email address before logging in. Check your inbox for the verification link.");
                    RequestDispatcher disp = request.getRequestDispatcher("login.jsp");
                    disp.forward(request, response);
                    return;
                }
    ...
    public String getServletInfo() {
        return "Login Servlet handling user database verification, email validation, and session profile setup including all CV sections.";
    }
}