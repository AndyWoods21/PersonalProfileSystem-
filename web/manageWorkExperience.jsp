...
<head>
    ...
    <title>Manage Work Experience — DevProfile</title>
    <style>
    :root {
...
<body>
    ...
    <header>
        ...
        <div class="user-nav">
            <span class="badge">Work Experience</span>
    ...
    <div class="app-container">
        <aside class="sidebar">
            ...
            <div>
                ...
                <ul class="nav-menu">
                    <li class="nav-item active"><a href="${pageContext.request.contextPath}/WorkExperienceServlet.do">Work Experience</a></li>
        ...
        <main>
            <div class="card">
                ...
                <form action="${pageContext.request.contextPath}/WorkExperienceServlet.do" method="post">
                    <div class="form-row">
                        <div class="form-group">
                            <label>Company Name</label>
                            <input type="text" name="companyName" class="form-control" required placeholder="e.g. Acme Corp">
                        </div>
                        <div class="form-group">
                            <label>Job Title</label>
                            <input type="text" name="jobTitle" class="form-control" required placeholder="e.g. Software Developer">
                        </div>
                    </div>
                    <div class="form-row">
                        <div class="form-group">
                            <label>Start Date</label>
                            <input type="text" name="startDate" class="form-control" placeholder="e.g. Jan 2023">
                        </div>
                        <div class="form-group">
                            <label>End Date</label>
                            <input type="text" name="endDate" class="form-control" placeholder="e.g. Present">
                        </div>
                    </div>
                    <div class="form-group">
                        <label>Description</label>
                        <textarea name="description" class="form-control" rows="3" placeholder="Describe your responsibilities..."></textarea>
                    </div>
            ...
            <div class="card">
                ...
                <c:choose>
                    <c:when test="${not empty requestScope.workList}">
                        <div class="item-list">
                            <c:forEach var="item" items="${requestScope.workList}">
                                <div class="item-card">
                                    <h3><c:out value="${item.jobTitle}" /> @ <c:out value="${item.companyName}" /></h3>
                                    <p style="color: #cccccc; font-size: 0.85rem; font-weight: 600; margin-top: 0.25rem;"><c:out value="${item.startDate}" /> - <c:out value="${item.endDate}" /></p>
                                    <p style="margin-top: 0.5rem; color: var(--text-body); font-size: 0.9rem;"><c:out value="${item.description}" /></p>
                                </div>
                            </c:forEach>
                        </div>
                    </c:when>
                    <c:otherwise>
                        <p class="empty-text">No work experience entries found. Add your first entry using the form above.</p>
                    </c:otherwise>
                </c:choose>
            </div>
        </main>
    ...
    <script>
        ...
        const techWords = ['JAVA', 'JSP', 'SERVLET', 'JDBC', 'HTML5', 'CSS3', 'SQL', 'MVC', 'TOMCAT', 'GIT', 'REST', 'OOP'];
        const activeWords = [];
        const maxWordsOnScreen = 18;
        ...
        class PopUpWord {
            ...
            reset() {
                ...
                this.opacity = 0;
                this.maxOpacity = Math.random() * 0.15 + 0.05; // Very subtle
        ...
        function animate() {
            // Fixed: Use clearRect instead of fillRect so the CSS background image isn't overwritten and blocked
